...
function NavFieldBuilder:delete_segment(id)
	...
	for i_vis_grp, vis_grp in ipairs(all_vis_groups)
	...
	for i_quad, _ in pairs(vis_grp.rooms)
	...
	slot13 = i_room
	slot14 = "vis_grp"
	slot15 = room.vis_group
	slot16 = "segment"
	slot17 = room.segment
	slot18 = "seg"
	--- END OF BLOCK #7 ---

	slot19 = if room.vis_group then
	JUMP TO BLOCK #8
	else
	JUMP TO BLOCK #9
	end
	...
	slot19 = all_vis_groups[room.vis_group].seg
	...
	slot12 = if test_seg.neighbours then
	JUMP TO BLOCK #20
	else
	JUMP TO BLOCK #21
	end
	...
	slot9 = if self._helper_blockers then
	JUMP TO BLOCK #33
	else
	JUMP TO BLOCK #37
	end
	...
	slot19 = if not all_rooms[i_quad] then
	JUMP TO BLOCK #40
	else
	JUMP TO BLOCK #41
	end
...
function NavFieldBuilder:_trash_room(i_room)
	...
	for i_vis_grp, vis_grp in ipairs(self._visibility_groups)
	...
	for old_i_room, _ in pairs(vis_grp.rooms)
...
function NavFieldBuilder:_analyse_room(enter_dir_str, enter_pos)
	...
	slot17 = if managers.navigation:get_debug_draw_state()
	 then
	JUMP TO BLOCK #4
	else
	JUMP TO BLOCK #5
...
function NavFieldBuilder:_create_room_to_room_visibility_data(build_data)
	...
	slot24 = if not old_rays.x_a then
	JUMP TO BLOCK #11
	else
	JUMP TO BLOCK #12
	end
	...
	slot25 = if not old_rays.y_a then
	JUMP TO BLOCK #13
	else
	JUMP TO BLOCK #14
	end
	...
	slot26 = if not old_rays.x_b then
	JUMP TO BLOCK #15
	else
	JUMP TO BLOCK #16
	end
	...
	slot27 = if not old_rays.y_b then
	JUMP TO BLOCK #17
	else
	JUMP TO BLOCK #18
	end
	...
	visibility, nr_raycasts, rays_x_a, rays_y_a, rays_x_b, rays_y_b = slot19(slot21, slot22, slot23, slot24, slot25, slot26, slot27, max_nr_raycasts - current_nr_raycasts, ray_dis)
	--- END OF BLOCK #18 ---
